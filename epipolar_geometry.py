...
def compute_fundamental(x1,x2):
    ...
    x1=[concatenate((i,[1]),axis=0) for i in x1]
    x2=[concatenate((i,[1]),axis=0) for i in x1]        
    x1=array(x1).T
    x2=array(x2).T
...
def plot_epipolar_line(img_path,F,x,epipole=None,show_epipole=False):
    ...
    # ndx = (lt>=0) & (lt<m) 
    a,b,c=dot(F,x)
    sample=linspace(0,n-1,100)
    e_line=array([(c+a*tt)/(-b) for tt in sample])
    ndx=(e_line>=0) & (e_line<m)
    
    return (sample[ndx], e_line[ndx])
...
def F_from_ransac(x1,x2,model,maxiter=5000,match_theshold=1e-6):
    """ Robust estimation of a fundamental matrix F from point 
        ...
        it: x1,x2 (3*n arrays) points in hom. coordinates. """
